...
import { Star, TrendingUp, Clock, DollarSign, Zap, Award, ArrowLeft, Sparkles, Gift, CheckCircle, Calculator, Calendar, CreditCard } from 'lucide-react';
...
export default function PassRecommendations({ onContinue, onBack }: PassRecommendationsProps) {
  const { selectedServices, selectedWaxCenter, addOrUpdatePass, selectedPasses } = useWaxPass();
  ...
  useEffect(() => {
    ...
    waxPasses.forEach(pass => {
      initialInstallments[pass.id] = pass.installmentOptions[0]?.id || '1-payment';
    });
  ...
  useEffect(() => {
    if (selectedPassId && selectedInstallments[selectedPassId]) {
      ...
      const selectedInstallment = selectedPass?.installmentOptions.find(opt => opt.id === selectedInstallments[selectedPassId]);
  ...
  const calculatePassPricing = (services: WaxService[], passLevel: string): { price: number, discountedPrice: number, savingsPerVisit: number, totalSavings: number } => {
    // Average service price for calculation
    const totalServicePrice = services.reduce((sum, service) => sum + (service.standard_price || 50), 0);
    ...
    switch (passLevel) {
      ...
      case '12U3': // 12 months unlimited, every 3 weeks = ~17 sessions  
        ...
        discountedPrice = avgServicePrice * 1.10;
        savingsPerVisit = avgServicePrice - discountedPrice;
  ...
  const generateInstallmentOptions = (passPrice: number): InstallmentOption[] => {
  ...
  const generatePasses = (): WaxPass[] => {
    ...
    const avgServicePrice = selectedServices.reduce((sum, service) => sum + (service.standard_price || 50), 0) / selectedServices.length || 50;
    ...
    passes.push({
      ...
      description: 'Perfect for first-time guests! 4 sessions with select services.',
      passType: 'prepaid',
      passLevel: 'B6G1',
      totalSessions: 4,
      freeSessions: 0,
      ...
      installmentOptions: generateInstallmentOptions(Math.round(avgServicePrice * 3))
    });
    ...
    passes.push({
      ...
      description: 'Great value for guests who visit once a month or less at one location.',
      passType: 'prepaid',
      passLevel: 'B9G2', 
      totalSessions: 12,
      freeSessions: 2,
      ...
      installmentOptions: generateInstallmentOptions(Math.round(singleCenterPricing.price * 0.9))
    });

    // Redeem Anywhere Pass - 12 or 7 sessions, never expires, flexibility premium
    const redeemAnywhere12Pricing = calculatePassPricing(selectedServices, 'B9G3');
    passes.push({
      id: 'redeem-anywhere-12', 
      name: 'Redeem Anywhere Pass (12)',
      description: 'Maximum flexibility! Use at any location with payment plan options.',
      passType: 'prepaid',
      passLevel: 'B9G3',
      totalSessions: 12,
      freeSessions: 3,
      ...
      installmentOptions: generateInstallmentOptions(redeemAnywhere12Pricing.price)
    });

    // Redeem Anywhere Pass - 7 sessions alternative
    const redeemAnywhere7Pricing = calculatePassPricing(selectedServices, 'B6G1');
    passes.push({
      ...
      installmentOptions: generateInstallmentOptions(redeemAnywhere7Pricing.price)
    });

    // UNLIMITED PASS (only for specific services)
    // Check if selected services qualify for unlimited (underarm, full face, brow, bikini)
    const unlimitedQualifyingServices = selectedServices.filter(service => 
    ...
    if (unlimitedQualifyingServices.length > 0) {
      ...
      passes.push({
        ...
        description: 'For guests who prefer to wax more frequently. 12 months of unlimited visits!',
        passType: 'unlimited',
        passLevel: '12U2',
  ...
  const getRecommendationIcon = (type: string) => {
    switch (type) {
      case 'best-value': return <TrendingUp className="h-4 w-4" />;
      case 'most-popular': return <Star className="h-4 w-4" />;
      case 'recommended': return <Award className="h-4 w-4" />;
  ...
  const getRecommendationColor = (type: string) => {
    switch (type) {
      case 'best-value': return 'from-green-500 to-emerald-500';
      case 'most-popular': return 'from-yellow-500 to-orange-500';
      case 'recommended': return 'from-blue-500 to-indigo-500';
  ...
  if (!selectedWaxCenter || selectedServices.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="glass-light border border-white/40 rounded-3xl p-12 max-w-md mx-auto">
          ...
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Incomplete Selection</h3>
          <p className="text-gray-600 mb-6">Please select a location and services first to see pass recommendations.</p>
          {onBack && (
            <Button 
              onClick={onBack}
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Choose Your Perfect Pass</h1>
              <p className="text-gray-600">Select a wax pass and payment plan that fits your needs</p>
            </div>
          </div>
                            
          {/* Summary Bar */}
          <Card className="glass-light border-white/40 mb-6">
            <CardContent className="p-4">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <p className="text-sm text-gray-500">Location</p>
                    <p className="font-semibold text-gray-800">{selectedWaxCenter?.display_name || 'Not selected'}</p>
                  </div>
                  <Separator orientation="vertical" className="h-8" />
                  <div className="text-center">
                    <p className="text-sm text-gray-500">Services</p>
                    <p className="font-semibold text-gray-800">{selectedServices.length} selected</p>
                  </div>
                  <Separator orientation="vertical" className="h-8" />
                  <div className="text-center">
                    <p className="text-sm text-gray-500">Total Value</p>
                    <p className="font-semibold text-gray-800">${selectedServices.reduce((sum, service) => sum + (service.standard_price || 50), 0)}</p>
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        {waxPasses.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {waxPasses.map((pass) => {
              ...
              const selectedInstallment = pass.installmentOptions.find(opt => opt.id === selectedInstallments[pass.id]);
              
              return (
                <Card 
                  key={pass.id}
                  className={`glass-card border-white/40 transition-all duration-300 cursor-pointer ${
                    isSelected 
                ...
                >
                  <CardHeader className="relative pb-4">
                    ...
                    <Badge 
                      className={`absolute -top-2 -right-2 glass-premium bg-gradient-to-r ${getRecommendationColor(pass.recommendationType)} text-white border-0 shadow-lg`}
                    >
                      {getRecommendationIcon(pass.recommendationType)}
                      <span className="ml-1 capitalize">{pass.recommendationType.replace('-', ' ')}</span>
                    </Badge>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      >
                        {isSelected && (
                          <div className="w-2 h-2 bg-white rounded-full mx-auto mt-[3px]"></div>
                        )}
                      </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="text-2xl font-bold text-gray-800">
                              ${selectedInstallment?.totalAmount || pass.currentPrice}
                            </div>
                            {selectedInstallment && selectedInstallment.installments > 1 && (
                              <div className="text-sm text-pink-600 font-medium">
                                ${selectedInstallment.amountPerInstallment} × {selectedInstallment.installments}
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {pass.validityMonths === 999 ? (
                        ...
                        <Badge variant="outline" className="text-xs border-orange-300 text-orange-700">
                          ⏰ {pass.validityMonths} Month{pass.validityMonths > 1 ? 's' : ''}
                        </Badge>
                      )}
                      
                      {pass.id.includes('redeem-anywhere') || pass.id === 'welcome' || pass.id === 'new-guest-12' || pass.id === '12u2-unlimited' ? (
                      ...
                      {pass.id !== 'single-center-12' && pass.id !== 'welcome' && pass.id !== 'new-guest-12' ? (
                    ...
                    <div className="glass-ultra-light p-4 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        {pass.installmentOptions.map((installment) => (
                          <button
                            key={installment.id}
                            onClick={() => handleInstallmentSelect(pass.id, installment.id)}
                          ...
                          >
                            <div className="font-medium">
                              {installment.installments === 1 ? '1 Payment' : `${installment.installments} Payments`}
                            </div>
